...
| Cluster | Agglomerative clustering (cosine, average linkage, tunable via `--threshold`) over advanced-but-unclustered signals. Uses OpenAI `text-embedding-3-small` vectors (threshold 0.6) when an `OPENAI_API_KEY` is set; otherwise TF-IDF (threshold 0.8) — keyless and local. Brute-force distances, no vector database. The richest member signal names the idea. |
...
| Keyless clustering is brittle | The TF-IDF fallback misses paraphrase-level relatedness (a paper and a changelog rarely share vocabulary) — exactly the cross-source clusters CPI cares most about. Embedding-based clustering (used automatically with an OpenAI key) measured markedly better on real signals: 18 advanced signals formed 9 coherent ideas versus 15 fragments under TF-IDF. |
