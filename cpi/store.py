...
from . import paths
from .models import FACTORS, CandidateIdea, Decision, SearchCriteria, SignalRecord, TriageResult
...
def _config_file(name: str) -> Path:
    """Home config with fallback to the packaged template (fresh installs)."""
    path = paths.config_dir() / name
    if path.exists():
        return path
    return paths.templates_dir() / "config" / name


def load_weights() -> dict[str, float]:
    with open(_config_file("weights.yaml"), "r", encoding="utf-8") as f:
...
def load_sources() -> dict:
    with open(_config_file("sources.yaml"), "r", encoding="utf-8") as f:
        return yaml.safe_load(f)


def source_scorecard() -> list[dict]:
    """Per-source triage outcomes - which sources earn their place in the config."""
    disp = {t.signal_id: t.disposition.value for t in iter_triage()}
    stats: dict[str, dict] = {}
    for s in iter_signals():
        d = disp.get(s.id)
        if d is None:
            continue
        row = stats.setdefault(s.source_name, {"advance": 0, "park": 0, "discard": 0})
        row[d] += 1
    out = []
    for source, row in stats.items():
        total = sum(row.values())
        out.append({"source": source, **row, "total": total,
                    "advance_rate": round(row["advance"] / total, 3) if total else 0.0})
    out.sort(key=lambda r: (-r["advance_rate"], r["source"]))
    return out


def source_health() -> list[dict]:
    """Per-source scan history summary from data/scan_log.jsonl (newest streaks)."""
    rows = read_jsonl(paths.scan_log_file())
    ...
    for source, runs in sorted(by_source.items()):
        ...
        last = runs[-1]
        out.append({
            "source": source, "runs": len(runs),
            "last_ts": str(last.get("ts", "")), "last_new": last.get("new", 0),
            "last_error": last.get("error"), "zero_streak": streak,
            "total_new": sum(r.get("new", 0) for r in runs),
        })
    return out


def load_search_criteria() -> SearchCriteria | None:
    """config/search.yaml, written by `cpi ground`. None -> scanners use PCM keywords."""
    path = paths.config_dir() / "search.yaml"
    if not path.exists():
        return None
    with open(path, "r", encoding="utf-8") as f:
        return SearchCriteria.model_validate(yaml.safe_load(f))


def save_search_criteria(criteria: SearchCriteria) -> Path:
    path = paths.config_dir() / "search.yaml"
    path.parent.mkdir(parents=True, exist_ok=True)
    header = (
        "# Search criteria generated by `cpi ground` from context/pcm.yaml "
        f"(PCM v{criteria.generated_from_pcm_version}).\n"
        "# Review and edit freely - scanners use this file when present and fall\n"
        "# back to the raw PCM theme keywords when it is absent.\n"
        "# Re-run `cpi ground --force` after significant PCM changes.\n"
    )
    body = yaml.safe_dump(criteria.model_dump(), sort_keys=False, allow_unicode=True)
