...
class ThemeSearch(BaseModel):
    """Per-source search vocabulary for one watch theme (generated by `cpi ground`)."""

    theme: str  # must match a WatchTheme.name
    arxiv_queries: list[str] = Field(default_factory=list)  # academic phrasing
    hn_keywords: list[str] = Field(default_factory=list)  # developer/community phrasing
    press_keywords: list[str] = Field(default_factory=list)  # trade-press phrasing


class SearchCriteria(BaseModel):
    """config/search.yaml - the searchable translation of the PCM.

    `standard_keywords` is derived mechanically (competitors, roadmap themes);
    `themes` is LLM-translated per source. Scanners fall back to the raw PCM
    theme keywords when this file is absent.
    """

    generated_from_pcm_version: str = ""
    standard_keywords: list[str] = Field(default_factory=list)
    themes: list[ThemeSearch] = Field(default_factory=list)

    def for_theme(self, name: str) -> Optional[ThemeSearch]:
        return next((t for t in self.themes if t.theme == name), None)

    def all_theme_terms(self, name: str) -> list[str]:
        t = self.for_theme(name)
        if t is None:
            return []
        return t.arxiv_queries + t.hn_keywords + t.press_keywords
